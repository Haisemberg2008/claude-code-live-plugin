...
server.registerTool('codeorquestra_start', { description: 'Inicia uma execução v2 na tarefa identificada pelo handle. O job segue o contrato v2 (contractVersion: 2). Opcional: limits { maxTokens, maxTurns, maxRuntimeSeconds } fixa um orçamento para esta execução; ao esgotar, o turno em andamento termina e nenhum outro é entregue (message/annotate respondem BUDGET_EXHAUSTED).', inputSchema: { taskHandle: handle.optional(), job: z.record(z.string(), z.unknown()), acknowledgeReview: z.boolean().optional(), observation: z.object({ mode: z.enum(['painel', 'voz']) }).optional().describe('Canal de acompanhamento. "painel" (padrao) exige uma aba do painel assinando os eventos desta tarefa; "voz" assume o acompanhamento narrado pelo coordenador.'), codexThreadId: z.string().optional().describe('Ignorado: nunca autoriza; use taskHandle.') } }, async ({ taskHandle, job, acknowledgeReview, observation }) => guarded(async () => {
